...
The primary data acquisition system will utilize camera sensors to collect images of sectors of parking spots in the lot behind Bell Hall. These images will then be sent to a server where the images will be processed by an artificial intelligence (AI) algorithm to determine the number of red parking spots that are filled and unfilled by cars. This subsystem is a critical subsystem because these cameras will provide the primary data to the system that will determine the count of vehicles in a lot. 

## Specifications

1. Minimum Field of View and Minimum Resolution

  * The cameras need to be able to have a field of view that could view all parking spots in Sector 1 through 5, as well as see a moving car right near the cameras at the entrances of Sector 6. The measurements used for the length and width of a car/parking spot are the measurements of a parking spot in Bell Hall, which were determined to be to be 18.87 ft x 8.69 ft (575 cm x 265 cm). The measurements of the minimum field of view needed for each sector (static sectors 1 to 5 and dynamic sector 6, with entrance camearas denoted as 6a and 6b) can be seen in Figure 1 below. Overall, a field of view of 98° is needed for this system.
  * For static Sectors 1 through 5, an overall minimum field of view of 96° is needed. The field of view was determined by finding the angle needed to see all of that sector's parking spots in entirety. In order, a field of view of 77°, 96°, 61°, 91°, and 75° are needed to see all of the parking spots in Sectors 1, 2, 3, 4, and 5, respectively. Thus, an FOV of 96° will  In addition, for all sectors, the minimum pixels per foot of a car at the furthest point in the interested range has been calculated, all of which are greater than the desired 10 pixels per foot (meaning that the AI algorithm has a good pixel per foot to work with to identify vehicles).
...
![Figure 1. Bell Hall Sectors Analysis](https://github.com/kesternucum/ECE-Capstone-Team3/blob/team-signoff-cameras/Documentation/Images/Bell_Hall_Lot_Sectors_Analysis.png)
...
![Figure 2. Bell Hall Parking Lot with Red Spots and Light Posts](https://github.com/kesternucum/ECE-Capstone-Team3/blob/team-signoff-cameras/Documentation/Images/Bell_Hall_Red_Spaces.png)
  ...
  * Minimum Resolution
    - The measurements for a single parking spot in the Bell Hall lot is 18.87 ft x 8.69 ft (575 cm x 265 cm). Based on these measurements the whole area of the lot is then calculated in Figure 3.  The computer vision algorithms implemented [1] and [2] which detect and/or identify vehicles used training set images of the rear side of cars that were 64 x 64 pixels. A standard car is on average 5.8 feet [3], which translates to roughly a little over 10 pixels per foot (or 100 pixels per square foot) for these images of cars to be identified as such. Thus, the cameras must have a minimum pixels per foot of 10 pixels per foot (or 100 pixels per/square foot) to identify the presence of a car in a parking spot or entrance at the furthest point from the camera that a car could be positioned.

3.  Monitoring of Parking Spaces and Entrances
  * Tracking Static Parking Spots
    - 5 cameras will need to be used to monitor individual parking spots in specified sectors (Sectors 1-5). For the sector in which it would be difficult to place cameras (Sector 6, in which the closest light post has a tree surrounding most of the light post), cameras will need to be placed at the entrances of that sector to monitor and track if a vehicle has entered or exited that area.

![Figure 3. Bell Hall Parking Lot Sectors](https://github.com/kesternucum/ECE-Capstone-Team3/blob/team-signoff-cameras/Documentation/Images/Bell_Hall_Sectors.png)
...
[Datasheet with Schematic of Camera](https://github.com/kesternucum/ECE-Capstone-Team3/blob/team-signoff-cameras/Documentation/3D%20Models/luxriot-vms-2022-11-22-172106.pdf)

[Mounting of Cameras on Light Posts](https://github.com/kesternucum/ECE-Capstone-Team3/blob/team-signoff-cameras/Documentation/3D%20Models/Camera_on_Light_Post.pdf)
...
* It can be seen above that each of the requirements in the specifications section are met with the chosen camera.
